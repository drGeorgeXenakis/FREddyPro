...
function (data, spd, dir, spdres = 2, dirres = 30, spdmin = 2, 
    ...
    labels = waiver(), title = NULL, xlab = "Direction", ylab = "Count", 
    fill.name = "Wind Speed (m/s)", size = list(title = 12, title.x = 10, 
        title.y = 10, text.x = 10, text.y = 10), debug = 0) 
{
    ...
    p.windrose <- ggplot(data = data, aes(x = dir.binned, fill = spd.binned)) + 
        geom_bar() + scale_x_discrete(drop = FALSE, labels = labels) + 
        coord_polar(start = -((dirres/2)/360) * 2 * pi) + scale_fill_manual(name = fill.name, 
        values = spd.colors, drop = FALSE) + theme(panel.background = element_blank(), 
        plot.title = element_text(colour = "black", size = size[["title"]], 
            hjust = 0.5), axis.title.x = element_text(colour = "black", 
            size = size[["title.x"]]), axis.title.y = element_text(colour = "black", 
            size = size[["title.y"]]), axis.text.x = element_text(colour = "black", 
            size = size[["text.x"]]), axis.text.y = element_text(colour = "black", 
            size = size[["text.y"]]), axis.ticks.x = element_line(colour = "black"), 
        axis.ticks.y = element_line(colour = "black"), panel.grid.major = element_line(colour = "lightgrey")) + 
